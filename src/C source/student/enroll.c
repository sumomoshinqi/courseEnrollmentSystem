...
int enroll(MYSQL *mysql)
{
    ...
    if (!mysql_num_rows(result)) {
        ...
        while ((row = mysql_fetch_row(result))) {
            ...
            memset(query, 0, sizeof(query));
            sprintf(query, "SELECT courseTimeslot FROM CourseEnrollment, CourseList WHERE CourseEnrollment.course = CourseList.courseID AND CourseEnrollment.student = \"%s\";", loginName);
            if (mysql_query(mysql, query)) {
                finishWithError(mysql);
            }
    ...
    memset(query, 0, sizeof(query));
    sprintf(query, "select * from CourseEnrollment where student=%s and course=\"%s\";", loginName, courseID);
    if (mysql_query(mysql, query)) {
        finishWithError(mysql);
    }
    result = mysql_store_result(mysql);
    ...
    // update course list
    memset(query, 0, sizeof(query));
    sprintf(query, "insert into CourseEnrollment values (\"%s\", %s);", courseID, loginName);
    if (mysql_query(mysql, query)) {
        finishWithError(mysql);
    }
